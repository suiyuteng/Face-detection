...
# path to wider face'
root_dir = r'D:\\Datasets\\MTCNN\\widerFace'
...
env_pos_label = lmdb.open(os.path.join(output_pos_dir, "label_pos"), map_size=LMDB_MAP_SIZE/100)

env_part_image = lmdb.open(os.path.join(output_part_dir, "image_part"), map_size=LMDB_MAP_SIZE)
env_part_label = lmdb.open(os.path.join(output_part_dir, "label_part"), map_size=LMDB_MAP_SIZE/100)

env_neg_image = lmdb.open(os.path.join(output_neg_dir, "image_neg"), map_size=LMDB_MAP_SIZE)
env_neg_label = lmdb.open(os.path.join(output_neg_dir, "label_neg"), map_size=LMDB_MAP_SIZE/100)
