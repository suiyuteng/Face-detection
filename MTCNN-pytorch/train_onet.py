...
def train():
    ...
    for epoch in range(start_epoch, max_iter + 1):
        ...
        if k % display == 0:
            ...
            log.info(
                "train iter: {}, lr: {}, loss: {:.4f}, cls loss: {:.4f}, bbox loss: {:.4f}, cls acc: {:.4f}, bbox acc: {:.4f}".format(
                    k, optimizer.param_groups[0]['lr'], loss.item(), loss_cls.item(), loss_reg.item(), acc_cls,
                    acc_reg))
